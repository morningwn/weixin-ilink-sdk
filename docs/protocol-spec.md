...
**URL:** `https://ilinkai.weixin.qq.com/ilink/bot/get_qrcode_status?qrcode=qrc_8f4b7b1d2cf74e98baf50a5d0cc4b4b7`
...
public static String randomWechatUin() {
    int value = new SecureRandom().nextInt() & 0xFFFFFFFFL; // unsigned
...
**URL:** `https://ilinkai.weixin.qq.com/ilink/bot/sendtyping`
...
**URL:** `https://ilinkai.weixin.qq.com/ilink/bot/getuploadurl`
...
**URL:** `https://novac2c.cdn.weixin.qq.com/c2c/upload?encrypted_query_param=AABQmM4mZ2Yx_d4mcpG4vincN2%3D&filekey=7cc7ad1d6aaf4c32b23dc4f8c40ec0cf`
...
sequenceDiagram
    ...
    B->>W: POST /ilink/bot/getupdates<br/>get_updates_buf=...
    W-->>B: ret=0, msgs[], context_token, get_updates_buf'
    B->>B: 持久化 get_updates_buf<br/>缓存(accountId,userId)->context_token
    opt 需要 typing
    ...
    A-->>B: 回复文本或媒体
    alt 回复文本
        B->>W: POST /ilink/bot/sendmessage<br/>context_token + TEXT item
    else 回复媒体
        B->>W: POST /ilink/bot/getuploadurl
        W-->>B: upload_param
        B->>W: POST CDN /upload<br/>AES-128-ECB ciphertext
        W-->>B: x-encrypted-param
        B->>W: POST /ilink/bot/sendmessage<br/>context_token + CDNMedia
    end
    opt 结束 typing
